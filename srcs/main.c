...
/*   Updated: 2019/05/09 00:23:20 by aben-azz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		init_termcaps(t_term *trm)
{
	...
	g_shell->tcap->carriage = tgetstr("cr", NULL);
...
static int		init_prompt(t_term *trm, char **env)
{
	struct winsize	*w;
...
int				main(int ac, char **av, char **env)
{
	...
	(void)ac;
	(void)av;
	if (!(tgetent(NULL, getenv("TERM"))) || init_termcaps(&term, env) == -1)
		return (-1);
	if (init_history() == -1)
		return (-1);
