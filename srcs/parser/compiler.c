...
void shearsh_pipe(t_ast *root,char *str, char **env)
{
	if (root != NULL) 
	{
		shearsh_pipe(root->left,ft_strjoin(str , " - > left"),env);
		if (root->token->op_type == PIPE)
			pipe_cmds(ft_strsplit(root->left->token->content,' '),
					ft_strsplit(root->right->token->content,' '),env);
		shearsh_pipe(root->right,ft_strjoin(str ,"- > right"), env);
	}
}
